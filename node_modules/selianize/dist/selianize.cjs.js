...
var StringEscape = _interopDefault(require('js-string-escape'));
    ...
    if (!options.skipStdLibEmitting) {
      ...
      if (suite.parallel) {
        testsCode = testsCode.map((code, index) => ({
          ...
          code: `${code.replace(/^it/, `jest.setTimeout(${suite.timeout * 1000});test`)}${hookResults}${snapshot ? snapshot.hook : ''}`
        }));
...
function emit$4(command, options = config, snapshot) {
  return new Promise(async (res, rej) => {
    if (emitters$2[command.command]) {
      ...
      try {
        let result = await emitters$2[command.command](preprocessParameter(command.target, emitters$2[command.command].target), preprocessParameter(command.value, emitters$2[command.command].value));
...
function emitControlFlowForEach(collectionVarName, iteratorVarName) {
  return Promise.resolve(`for (let i = 0; i < vars["${collectionVarName}"].length - 1; i++) {vars["${iteratorVarName}"] = vars["${collectionVarName}"][i];`);
}
...
function emitSetWindowSize(size) {
  ...
  return Promise.resolve(`await driver.manage().window().setRect({ width: ${width}, height: ${height} });`);
}
